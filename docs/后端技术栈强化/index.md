...
> 面向字节「后端 agent 决策」岗位的后端能力补齐。按 **S1 → S10** 依赖顺序学（S7 为工程化底座加分项，S8 为分布式理论与能力强化兜底，S9 为海量素材落地，S10 为 AI 应用的向量检索底座），共约 19 周。
...
S1 MySQL（数据层）→ S2 Redis（缓存层）→ S3 Kafka（异步层）
        ...
        → S7 K8s（工程化底座）→ S8 分布式理论（共识与能力强化兜底）
...
> 分布式基础理论 + **能力强化系列**（导师清单：降级方案/底层存储/限流熔断/gRPC 平衡性/中间件/HA/选主/Raft 数据同步/容灾）。先读[能力全景](./08-distributed/分布式系统能力全景)立骨架，再按"理论 → 共识 → 协调 → 能力 → 收口"顺序学。与 S4 微服务、S7 K8s（etcd）互相印证。配套：[第二阶段 · 分布式系统面试详解](/第二阶段-知识详解/分布式系统面试详解)（考点凝练背诵版）。

| 文章 | 内容 |
|------|------|
| [分布式系统能力全景](./08-distributed/分布式系统能力全景) | **导师清单总纲**：六层能力地图（通信/存储/一致性/协调/治理/高可用容灾）、中间件地图、能力×故障对照表 |
| [CAP 与 BASE 理论](./08-distributed/CAP与BASE理论) | CAP 证明与误区、PACELC、CP/AP 选型、最终一致落地模式 |
| [底层存储与数据同步](./08-distributed/底层存储与数据同步) | 副本模型、复制日志（binlog/AOF/Raft log）、WAL 与崩溃恢复、B+树 vs LSM-Tree、分片与一致性哈希 |
...
| [选主机制详解](./08-distributed/选主机制详解) | 方案谱系（DB/Redis/etcd/Raft）、租约选举实现、TTL 权衡、fencing token、接管与让位、K8s/Kafka/Milvus 对照 |
| [gRPC 负载均衡详解](./08-distributed/gRPC负载均衡详解) | 客户端 LB 三件套（resolver/balancer/subchannel）、策略对比（P2C 生产主流）、长连接失衡与健康检查、Go 实现 |
| [网络通信与降级方案](./08-distributed/网络通信与降级方案) | 网络故障模型、全链路超时、重试与退避/重试风暴、降级分类与开关、舱壁隔离 |
| [限流与熔断](./08-distributed/限流与熔断) | 五种限流算法、Redis+Lua 分布式限流、熔断器状态机与参数、防雪崩组合拳 |
| [高可用机制全景](./08-distributed/高可用机制全景) | 可用性指标（几个 9/MTBF/MTTR）、四大支柱（冗余/检测/转移/恢复）、全组件 HA 对照表、无状态化 |
| [容灾与备份恢复](./08-distributed/容灾与备份恢复) | RPO/RTO、备份体系与 PITR、同城双活/两地三中心/三地五中心、切换回切与演练 |
| [面试题集](./08-distributed/面试题集) | 10 题追问 3 层 + 6 题能力强化补充题 |
...
学完 S1~S10 的**组件原理**后，进入[架构师修炼](/架构师修炼/)补齐**演进决策 + 落地参数 + 故障兜底**——S 系列讲「组件是什么」，该栏目讲「什么时候用、怎么配、挂了怎么办」：

| 主题 | 对应篇章 |
|------|----------|
| 从单体到百万 QPS 的六道坎 | [01 QPS 分级与架构演进地图](/架构师修炼/01-QPS分级与架构演进地图) |
| MySQL 主从 / 半同步 / 主从延迟治理 | [03 MySQL 主从与读写分离落地](/架构师修炼/03-MySQL主从与读写分离落地) |
| Redis 哨兵 / Cluster 与丢写边界 | [04 Redis 高可用与缓存体系落地](/架构师修炼/04-Redis高可用与缓存体系落地) |
| Kafka 可靠性参数与积压处置 | [05 Kafka 削峰与可靠投递落地](/架构师修炼/05-Kafka削峰与可靠投递落地) |
| 分库分表与不停机迁移双写 | [06 分库分表与在线迁移双写](/架构师修炼/06-分库分表与在线迁移双写) |
| 缓存一致性 / 分布式事务 / 幂等 | [08](/架构师修炼/08-缓存与DB一致性落地) · [09](/架构师修炼/09-分布式事务与最终一致落地) · [11](/架构师修炼/11-幂等去重与ExactlyOnce) |
| etcd 选主、Lease 与 fencing 防双写 | [10 etcd 与 Raft 选主租约落地](/架构师修炼/10-etcd与Raft选主租约落地) |
| 白板面试 + 15 个可复现实验 | [17 面试实战](/架构师修炼/17-面试-白板架构设计与追问链) · [18 实验手册](/架构师修炼/18-实验手册-Go落地实验)（配套 `code/architect/` 编排） |
| 性能瓶颈定位（pprof 取证） | [19 pprof 实战](/架构师修炼/19-pprof实战/)：观测决策图 · CPU 火焰图 · 内存与 GC · goroutine 与锁阻塞 · 常见问题手册 · 生产采集 · 7 个案例 · 30 道面试题（配套 `code/architect/pprof-lab/` 8 个零依赖实验） |

## 待定（backlog）

分布式进阶（分库分表实战 / 分布式事务深入）—— 已由[架构师修炼](/架构师修炼/)栏目承接，并配套 `code/architect/`（MySQL 主从 / Redis 主从+哨兵 / etcd 三节点 / Kafka 集群）实验编排。（分布式理论已由 S8 覆盖：能力全景、CAP/BASE、底层存储（含一致性哈希/分片）、Raft、etcd、选主、gRPC 负载均衡、网络降级、限流熔断、高可用、容灾；etcd/分布式锁/RPC 治理的 Go 手写实践见 [路线专题 Day 8-9 代码集](/习题集和答案/route/)。）
