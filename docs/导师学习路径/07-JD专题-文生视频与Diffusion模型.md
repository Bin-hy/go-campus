...
> 为什么学：JD 写了「**文生视频、图生视频**」。你在 shatangAI 里调度过的视频生成通道（火山 Ark / Seedance 2.0，2026-08 起对新任务停用；当前新任务走腾讯云 VOD AIGC），以及阿里云 DashScope 的出图与分镜理解，背后都是扩散模型，面试官大概率会追问一句"这类模型到底是怎么生成视频的"。这一章把扩散模型讲透，让你从"会调 API"升级到"懂原理"。
> **口径提醒**：讲项目时以 [11 章 · 第 7 节「把自己挂上去」](./11-JD专题-生成原理零基础学习方案) 为准（历史接入 6 家供应商 / 当前新任务只走腾讯云 VOD 1 家；DashScope 是分镜理解与出图，不是视频生成通道）。
> **如果你觉得本章读起来吃力**：先读 [11 · 生成原理零基础学习方案](./11-JD专题-生成原理零基础学习方案)，它用大白话铺好地基并给出每题的口述稿，读完再回本章看推导会顺畅很多。
...
| Seedance 和 Sora 的区别？ | 都是扩散 + 时空建模路线；Seedance 是**字节跳动**自研（即梦/Dreamina/剪映生态的底层视频模型，多版本迭代快），Sora 是 OpenAI 的标杆实现。注意别把 Seedance 说成阿里的——阿里的是通义万相 Wan |
...
- [11 · 生成原理零基础学习方案](./11-JD专题-生成原理零基础学习方案)：**本章的入门版**（推荐先读）——术语扫盲、7 天日程、8 层递进讲解、12 题作答卡（含 60 秒口述稿）、20 题自测卷、项目口径雷区表；**不含代码、不用装环境**
