...
**阅读约定**：`1808N` / `1908N` 指本栏目样例的**业务端口** / **pprof 管理端口**（= 业务端口 + 1000，如 L01 → 18081 / 19081）；代码在 `code/perf/pprof-lab`（module `gocampus/perf/pprof-lab`，go 1.22，仅标准库），`-fix=true` 启动修复实现用于前后对比；命令默认已 `cd code/perf/pprof-lab`。
