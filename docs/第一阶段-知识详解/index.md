...
本页是**总览**，适合先通读建立知识骨架；下列 10 篇**专题精读**针对单点深挖（含问题推导、版本边界与常见误区），建议对照阅读。

| 专题 | 解决什么问题 |
| --- | --- |
| [Slice、Map 与内存布局](/第一阶段-知识详解/Slice-Map与内存布局) | slice header 与扩容、map 桶结构与并发安全边界 |
| [Interface 底层原理](/第一阶段-知识详解/Interface底层原理) | iface/eface、itab 缓存、nil 接口陷阱 |
| [String 与字节切片](/第一阶段-知识详解/String与字节切片) | 不可变性、零拷贝转换与拼接代价 |
| [Go 函数调用与栈详解](/第一阶段-知识详解/Go函数调用与栈详解) | PC/SP、Stack Frame、逃逸与 goroutine 栈伸缩 |
| [Go 内存分配详解](/第一阶段-知识详解/Go内存分配详解) | mcache / mcentral / mheap / mspan / size class |
| [Go GC 详解](/第一阶段-知识详解/Go GC 详解) | 三色标记、写屏障、GC 触发与调优 |
| [Go 并发编程详解](/第一阶段-知识详解/Go并发编程详解) | goroutine、channel、sync 原语与并发模式 |
| [Go GMP 调度详解](/第一阶段-知识详解/Go GMP 调度详解) | GMP 关系、G 状态机、Park / Handoff 与抢占 |
| [Go 并发编程问答集](/第一阶段-知识详解/Go并发编程问答集) | 并发场景面试问答速查 |
| [Go Context 详解](/第一阶段-知识详解/Go Context 详解) | 取消传播源码、超时约束与最佳实践 |

> 配套练习：[第一阶段 34 道练习](/习题集和答案/phase1/) · 阶段计划：[第一阶段：Go 语言深入](/学习计划安排/第一阶段-Go语言深入)
...
::: tip 专题精读
本章已整理为独立专题：[Go GC 详解](/第一阶段-知识详解/Go GC 详解)。专题补充了漏标产生的经典场景、插入/删除写屏障与混合写屏障的准确理解、GC Pacer 与 GC Assist，以及完整面试回答稿，适合在阅读本页总览后继续学习。
:::
