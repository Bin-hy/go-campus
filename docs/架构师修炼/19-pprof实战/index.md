...
| [架构师修炼 13](../13-容量规划压测与故障演练) | 容量模型、压测、演练**方法论** | 本栏目提供压测与采集**同时进行**的具体操作（`code/perf/pprof-lab/cmd/load` 自带压测器） |
...
## 五、实验清单：`code/perf/pprof-lab/`

所有实验都在 [`code/perf/pprof-lab/`](https://github.com/Bin-hy/go-campus/tree/main/code/perf/pprof-lab)：**独立 Go module、零第三方依赖、只用标准库**，`go run` 直接跑，不需要 Docker、不需要 wrk（自带压测器 `cmd/load`）。
...
- 代码实验包：[`code/perf/pprof-lab/`](https://github.com/Bin-hy/go-campus/tree/main/code/perf/pprof-lab)
