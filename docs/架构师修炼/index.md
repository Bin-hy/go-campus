...
| **M2 会定位瓶颈** | 给定「P99 从 50ms 涨到 800ms」，能列出 6 条排查路径并排出优先级 | [02](./02-单体架构的极限与分层) · [13](./13-容量规划压测与故障演练) |
...
- **配合**：[K8s Code 教程](/k8s-code教程/) 解决「部署与编排」，本栏目解决「架构与数据」。
- **产出**：[18 实验手册](./18-实验手册-Go落地实验)里的实验可以直接变成简历上的项目经历——**「我用 Go 实现了带主从切换与对账兜底的订单链路，并压测验证了 X 万 QPS 下的丢数据窗口为零」**，比「熟悉 Redis」强一百倍。
