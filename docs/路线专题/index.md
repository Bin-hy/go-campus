...
> 四大板块并行推进：**算法专项训练**贯穿全周期；**后端与计算机基础夯实**（Day 1-7 主线，Day 8 etcd/RPC 强化日弹性安排）、**大模型与 Agent 核心能力构建**（Day 8-17）、**简历项目改造与实战演练**（Day 18-30）三段主线接力。
...
flowchart TB
    ...
    subgraph 主线["三阶段主线"]
        B1["Day 1-7<br/>后端与计算机基础<br/>语言 / OS / 网络 / 存储<br/>+ Day8 强化: etcd / RPC"]
...
| [后端与计算机基础夯实（Day 1-8）](./02-后端与计算机基础) | Day 1-7（+8 强化） | Go/Python 并发、OS/网络、MySQL/Redis/MQ/向量库；Day 8：etcd（Raft/Watch/Lease/分布式锁）+ RPC/gRPC（超时/重试/熔断） | 计划「二」 |
...
| Day 7 | 链表综合 + LRU | 综合复习与自测（40 题，含 etcd/RPC 强化 10 题） | 7 天复盘表 |
...
>
> 强化日说明：**Day 8（etcd 与 RPC）**是后端主线的弹性加练日，建议安排在首个休息日或与 AI 主线 Day 8-17 并行推进（每天 +1~1.5h），内容与代码练习见 [02-后端与计算机基础](/路线专题/02-后端与计算机基础#day-8-强化日-etcd-与-rpc-分布式协调与远程调用)。
...
- **代码练习**：/练习指南、code/ 目录（Go 练习题）；Day 1-8 实战代码集见 `code/route/`（[习题集和答案/route](/习题集和答案/route/)，含 Day 8 etcd 分布式锁与 RPC 客户端治理）
