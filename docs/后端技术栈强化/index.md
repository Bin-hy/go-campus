...
分布式进阶（分库分表实战 / 分布式事务深入）—— 后续单独规划。（分布式理论已由 S8 覆盖：能力全景、CAP/BASE、底层存储（含一致性哈希/分片）、Raft、etcd、选主、gRPC 负载均衡、网络降级、限流熔断、高可用、容灾；etcd/分布式锁/RPC 治理的 Go 手写实践见 [路线专题 Day 8-9 代码集](/习题集和答案/route/)。）
