...
15. 想补上「性能瓶颈定位」这块最容易被面试官追问的能力：进入子栏目 [19 pprof 实战](/架构师修炼/19-pprof实战/)——按「[观测体系与原理](/架构师修炼/19-pprof实战/01-观测体系与pprof原理) → [CPU 火焰图](/架构师修炼/19-pprof实战/02-CPU火焰图实战) / [内存与 GC](/架构师修炼/19-pprof实战/03-内存与GC实战) / [goroutine 与锁阻塞](/架构师修炼/19-pprof实战/04-goroutine与锁阻塞实战) → [常见问题手册](/架构师修炼/19-pprof实战/05-常见问题排查手册) → [生产环境实践](/架构师修炼/19-pprof实战/06-生产环境pprof实践) → [实战案例集](/架构师修炼/19-pprof实战/07-实战案例集) → [面试题库](/架构师修炼/19-pprof实战/08-面试题与追问链)」推进，配套 [`code/perf/pprof-lab/`](https://github.com/Bin-hy/go-campus/tree/main/code/perf/pprof-lab) 的 8 个零依赖实验（自带压测器，`-fix` 一键对比修复前后）亲手跑出火焰图差异。
