...
2. 阅读[第一阶段知识点总览](/第一阶段-知识详解/)，建立 Go 核心知识框架。
...
10. 补齐后端短板：进入[后端技术栈强化](/后端技术栈强化/)模块，按 MySQL → Redis → Kafka → 微服务 → 高并发场景题 → Agent Backend 顺序学习，最后补上工程化底座 K8s（加分项）与[向量数据库 Milvus](/后端技术栈强化/10-milvus/01-向量检索原理)（AI 检索底座）。
...
13. 拿到导师的《Agent 学习路径》后，进入[导师学习路径](/导师学习路径/)——已把原始提纲提炼为 11 个章节的知识地图（RAG 原理 → Prompt 工程 → Function Calling/MCP → ReAct → Multi-Agent → 记忆/评测 → Diffusion → 推理优化 → 训练流程 → 技术栈 → [生成原理零基础学习方案](/导师学习路径/11-JD专题-生成原理零基础学习方案)），每章含知识点提炼、mermaid 流程图、面试问答（带参考答案）与自测清单，按 Day 1-7 主线排布可在一周内过完第一遍。**机器学习零基础的话从第 11 章开始**：它是图像/视频生成原理的入门方案——术语扫盲、7 天日程、8 层递进讲解、12 题作答卡（每题配 60 秒口述稿与追问应对）、20 题自测卷，**不含代码、不用装环境**；打牢地基后再读第 7 章的原理推导。
...
15. 想补上「性能瓶颈定位」这块最容易被面试官追问的能力：进入子栏目 [19 pprof 实战](/架构师修炼/19-pprof实战/)——按「[观测体系与原理](/架构师修炼/19-pprof实战/01-观测体系与pprof原理) → [CPU 火焰图](/架构师修炼/19-pprof实战/02-CPU火焰图实战) / [内存与 GC](/架构师修炼/19-pprof实战/03-内存与GC实战) / [goroutine 与锁阻塞](/架构师修炼/19-pprof实战/04-goroutine与锁阻塞实战) → [常见问题手册](/架构师修炼/19-pprof实战/05-常见问题排查手册) → [生产环境实践](/架构师修炼/19-pprof实战/06-生产环境pprof实践) → [实战案例集](/架构师修炼/19-pprof实战/07-实战案例集) → [面试题库](/架构师修炼/19-pprof实战/08-面试题与追问链)」推进，配套 [`code/architect/pprof-lab/`](https://github.com/Bin-hy/go-campus/tree/main/code/architect/pprof-lab) 的 8 个零依赖实验（自带压测器，`-fix` 一键对比修复前后）亲手跑出火焰图差异。
16. 面试前收口：进入[面试项目深挖](/面试项目深挖/)栏目——三个可写进简历的项目各一套深挖包（[Vistack 分布式视频平台](/面试项目深挖/Vistack/) · [BinRag RAG 知识库问答](/面试项目深挖/BinRag/) · [EasyCoding 终端 AI Agent](/面试项目深挖/EasyCoding/)），每套都含三版口述稿、技术深挖问答、压力面话术、速背卡片，以及**「诚实边界（不能吹的点）」**；[栏目总览](/面试项目深挖/)里说明了这一场该用哪个项目唱主角。
